...
export function RankingChart({ history, maxSeries = 5 }) {
  ...
  const { periods, lines, yScale, xScale } = useMemo(() => {
    if (!history || history.length < 2) return { periods: [], lines: [], yScale: () => 0, xScale: () => 0 }

    // Nhãn các kỳ (trục X)
    const periods = history.map((h) => h.periodLabel)

    // Lấy danh sách seriesId duy nhất, giới hạn theo maxSeries
    const seriesIds = [...new Set(history.flatMap((h) => h.entries.map((e) => e.seriesId)))].slice(0, maxSeries)

    // Tìm rank cao nhất (số lớn nhất) để xác định miền giá trị trục Y
    const maxRank = Math.max(...history.flatMap((h) => h.entries.map((e) => e.rank)), 10)
    const minRank = 0

    // Hàm tỉ lệ: chuyển rank → toạ độ Y (rank 1 ở gần đỉnh)
    const yScale = (rank) => PAD.top + ((rank - 1) / (maxRank - 1)) * (H - PAD.top - PAD.bottom)

    // Hàm tỉ lệ: chuyển index kỳ → toạ độ X
    const xScale = (i) => PAD.left + (i / (periods.length - 1)) * (W - PAD.left - PAD.right)
    ...
    const lines = seriesIds.map((sid, idx) => {
      ...
      const entry = history[history.length - 1]?.entries.find((e) => e.seriesId === sid)
  ...
  return (
    <div className="overflow-x-auto">
      {/* SVG Chart */}
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[600px]" style={{ minHeight: H }}>
        {/* Trục X: nhãn các kỳ */}
        {periods.map((label, i) => (
          <text
            key={i}
        ...
        {[1, 3, 5, 10].filter((r) => r <= Math.max(...history.flatMap((h) => h.entries.map((e) => e.rank)), 10)).map((r) => (
          <g key={r}>
            <text
              x={PAD.left - 6}
              y={yScale(r) + 3}
        ...
        {lines.map((line) => (
          <button
            ...
            className={cn(
              ...
              highlighted && highlighted !== line.seriesId ? 'opacity-40' : 'opacity-100',
            )}
          >
            <span className="w-2.5 h-2.5 rounded-full" style={{ background: line.color }} />
            <span className="text-on-surface-variant/70">{line.seriesTitle}</span>
          </button>
        ))}
      </div>
    </div>
