...
import { Search, Bell, LogOut, User as UserIcon, Settings, X } from 'lucide-react'
      ...
      <div className="flex items-center gap-4 min-w-0">
        {showSearch ? (
          ...
          <form onSubmit={handleSearch} className="flex items-center gap-2 flex-1">
            ...
            />
            <button type="button" onClick={() => { setShowSearch(false); setSearchQuery('') }} className="text-on-surface-variant hover:text-on-surface">
              <X size={16} />
            </button>
          </form>
        ) : (
          // Tiêu đề trang
          <h1 className="text-lg font-semibold text-on-surface truncate">{title || 'Dashboard'}</h1>
        )}
      </div>
          ...
          <DropdownMenu
            trigger={
              <button className="flex items-center gap-2.5 ml-2 px-2 py-1.5 hover:bg-black/[0.05] transition-all">
                ...
                <span className="text-sm font-medium text-on-surface hidden sm:block">{user.displayName}</span>
              </button>
            }
            align="end"
          >
            ...
            <DropdownMenuItem icon={<UserIcon size={16} />} onClick={() => navigate('/profile')}>Profile</DropdownMenuItem>
            <DropdownMenuItem icon={<Settings size={16} />} onClick={() => navigate('/profile')}>Settings</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem icon={<LogOut size={16} />} danger onClick={logout}>Sign Out</DropdownMenuItem>
          </DropdownMenu>
        )}
      </div>
    </header>
