...
import { X, Check, AlertCircle, Clock, ArrowUp, MessageSquare, FileText } from 'lucide-react'
        ...
        {userNotifications.length === 0 ? (
          ...
          userNotifications.map((n) => {
            ...
            return (
              <button
                key={n.id}
                onClick={() => { onClose(); n.referenceType === 'CHAPTER' && navigate(`/workspace/${n.referenceId}`) }}
                className={cn(
              ...
              >
                ...
                <div className={cn(
                  ...
                  !n.isRead ? 'border-primary bg-primary/5' : 'border-border-light',
                )}>
                  <Icon size={14} className={!n.isRead ? 'text-primary' : 'text-on-surface-variant'} />
                </div>
                {/* Nội dung thông báo */}
                <div className="flex-1 min-w-0">
                  <p className={cn('text-sm leading-snug', !n.isRead ? 'font-medium text-on-surface' : 'text-on-surface-variant')}>
                    {n.title}
                  </p>
                  {n.message && <p className="text-xs text-on-surface-variant mt-0.5 line-clamp-2">{n.message}</p>}
                  <p className="text-[10px] text-on-surface-variant/60 mt-1">{formatRelativeTime(n.createdAt)}</p>
                </div>
              </button>
            )
          })
