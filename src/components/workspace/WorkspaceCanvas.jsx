...
import { Stage, Layer, Rect, Text, Transformer, Group, Image as KonvaImage, Circle, Line } from 'react-konva'
...
import { getLayerPlaceholder } from '../../lib/page-placeholder'
...
function LayerImage({ layer, pw, ph }) {
  ...
  if (layer.fileUrl && img) {
    return <KonvaImage image={img} x={0} y={0} width={pw} height={ph} opacity={layer.opacity} listening={false} />
  }
  const placeholder = getLayerPlaceholder(layer.label || '', pw, ph, layer.opacity)
  return <KonvaImage image={placeholder} x={0} y={0} width={pw} height={ph} opacity={layer.opacity} listening={false} />
}
...
export function WorkspaceCanvas() {
  ...
  const pageImage = usePageImage(page?.originalImageUrl || page?.webImageUrl || undefined)
   ...
   */
  const handleReplySubmit = useCallback((parentComment) => {
    ...
    addComment(reply)
    updateComment(parentComment.id, { replyCount: (parentComment.replyCount || 0) + 1 })
  ...
  const handleRegionDragMove = useCallback(
    (e, region) => {
      updateRegion(region.id, { x: Math.round(e.target.x()), y: Math.round(e.target.y()) })
    },
    [updateRegion],
  )
  ...
  const handleMouseMove = useCallback(
    (_e) => {
      ...
      switch (mode) {
        case 'draw':
          if (!drawStart) return
          setDrawCurrent({ x: Math.round(Math.max(0, Math.min(pw, pos.x))), y: Math.round(Math.max(0, Math.min(ph, pos.y))) })
          break
        case 'pen':
          if (!penPoints) return
          setPenPoints([...penPoints, Math.round(Math.max(0, Math.min(pw, pos.x))), Math.round(Math.max(0, Math.min(ph, pos.y)))])
          break
        case 'highlight':
          if (!highlightStart) return
          setHighlightCurrent({ x: Math.round(Math.max(0, Math.min(pw, pos.x))), y: Math.round(Math.max(0, Math.min(ph, pos.y))) })
          break
      }
    },
  ...
  const handleMouseUp = useCallback(
      ...
      switch (mode) {
        ...
        case 'highlight':
          if (highlightStart && highlightCurrent && currentPageId) {
            ...
            const hw = Math.max(Math.abs(highlightCurrent.x - highlightStart.x), 10)
            const hh = Math.max(Math.abs(highlightCurrent.y - highlightStart.y), 10)
            addAnnotation({
              id: ++annotationIdCounter,
              pageId: currentPageId,
              type: 'highlight',
    ...
    [mode, commentPos, pw, ph, drawStart, drawCurrent, penPoints, highlightStart, highlightCurrent, currentPageId, addRegion, addAnnotation, penColor, getPos],
  )
  ...
  const handleKeyDown = useCallback(
    (e) => {
      if (e.key === 'Escape') {
        if (selectedCommentId) { selectComment(null); return }
        if (selectedAnnotationId) { selectAnnotation(null); return }
        if (drawStart) { setDrawStart(null); setDrawCurrent(null); return }
        if (commentPos) { setCommentPos(null); setCommentText(''); return }
        if (penPoints) { setPenPoints(null); return }
        if (annotationTextPos) { setAnnotationTextPos(null); setAnnotationTextValue(''); return }
        if (highlightStart) { setHighlightStart(null); setHighlightCurrent(null); return }
        return
      }
    ...
    [selectedRegionId, selectedCommentId, selectedAnnotationId, mode, deleteRegion, deleteAnnotation, drawStart, commentPos, penPoints, annotationTextPos, highlightStart, selectComment, selectAnnotation, addToast],
  )
  ...
  if (!currentPageId) {
    return (
      <div ref={containerRef} className="flex items-center justify-center h-full min-h-[300px]">
        <p className="text-sm text-workspace-text-secondary/60">Select a page to begin</p>
      </div>
    )
  }
    ...
    >
      ...
      >
        <Layer>
          ...
          {page?.originalImageUrl && pageImage && (
            <KonvaImage image={pageImage} x={0} y={0} width={pw} height={ph} listening={false} />
          )}
          ...
          {[...layers]
            ...
            .map((layer) => {
              ...
              return <LayerImage key={`layer-${layer.id}`} layer={layer} pw={pw} ph={ph} />
            })}
          ...
          {highlightStart && highlightCurrent && (
            <Rect
              ...
              width={Math.max(Math.abs(highlightCurrent.x - highlightStart.x), 1)}
              height={Math.max(Math.abs(highlightCurrent.y - highlightStart.y), 1)}
              fill="#ffeb3b"
              opacity={0.3}
              stroke="#fdd835"
              strokeWidth={1 / scale}
          ...
          {annotations.map((a) => {
            ...
            if (a.type === 'pen') {
              return (
                <Line
                  ...
                  onClick={(e) => {
                    e.cancelBubble = true
                    if (mode === 'select') selectAnnotation(selectedAnnotationId === a.id ? null : a.id)
                  }}
                />
              )
            }
            ...
            if (a.type === 'text') {
              const ts = 16 / scale
              const tw = Math.max((a.text?.length || 5) * ts * 0.65, 80 / scale)
              const th = 28 / scale
              return (
      ...
      {annotationTextPos && (() => {
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg shadow-xl"
              style={{ backgroundColor: '#1a1a2e', border: '1.5px solid #9ca3af' }}>
              <input
                autoFocus
                ...
                style={{ minWidth: 160, width: Math.max(160, annotationTextValue.length * 10 + 28) }}
              />
              <button
                onClick={() => {
