...
import { useSeriesDetail, useChapterDetail, usePagesByChapter } from '../hooks/useMockData'
...
import { ChevronLeft, BookOpen, Image, Calendar, Hash, BarChart3, Loader, ExternalLink, MessageSquare, Edit } from 'lucide-react'
...
export function ChapterDetailPage() {
  ...
  const { data: series, isLoading: seriesLoading } = useSeriesDetail(Number(seriesId))
  const { data: chapter, isLoading: chapterLoading } = useChapterDetail(Number(chapterId))
  const { data: pages, isLoading: pagesLoading } = usePagesByChapter(Number(chapterId))
  ...
  if (!series || !chapter) {
    return (
      <EmptyState
        ...
        action={<Button variant="ghost" onClick={() => navigate(`/series/${seriesId}`)}>Back to Series</Button>}
      />
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-on-surface-variant/70 mt-1">{series.title}</p>
        </div>
          ...
          {isMangaka && (
            <>
              ...
              <Link to={`/workspace/${chapterId}`}>
                <Button variant="primary" size="sm"><ExternalLink size={14} /> Open Workspace</Button>
              </Link>
            </>
          )}
          {isEditor && (
            <Link to={reviewHref}>
              <Button variant="primary" size="sm"><MessageSquare size={14} /> Open in Review</Button>
            </Link>
          )}
        </div>
      </div>
          ...
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-[10px] text-on-surface-variant/60 uppercase tracking-wider">
                {chapter.deadline ? 'Deadline' : chapter.publishedAt ? 'Published' : 'Created'}
              </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {paginatedPages.map((page) => (
              <Link key={page.id} to={`/workspace/${chapterId}`} className="group block">
