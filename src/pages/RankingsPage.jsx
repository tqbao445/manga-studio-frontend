...
import { TrendingUp, TrendingDown, Minus, Trophy, BarChart3, AlertTriangle, Siren } from 'lucide-react'
...
function getTrendColor(trend) {
  switch (trend) {
    ...
    case 'DOWN': return 'text-status-danger'
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-on-surface-variant/70 mt-1">Current period: {sorted[0]?.periodLabel || 'N/A'}</p>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 text-sm font-medium text-on-surface flex items-center gap-2">
                    {r.seriesTitle}
                    {wl >= 3 && <Siren size={14} className="text-status-danger" title="Cancellation review" />}
                    {wl === 2 && <AlertTriangle size={14} className="text-status-warning" title="Final warning" />}
                    {wl === 1 && <AlertTriangle size={14} className="text-status-warning/60" title="Warning issued" />}
                  </span>
                  {/* Tier */}
                  <span className="w-16 text-center">
                    <span
                      className="inline-block px-2 py-0.5 text-xs font-bold border"
                      style={{ borderColor: getRankColor(r.tier), color: getRankColor(r.tier), background: `${getRankColor(r.tier)}08` }}
                    >
                      {r.tier}
                    </span>
                  </span>
                  {/* Tổng vote */}
                  <span className="w-28 text-right text-sm text-on-surface-variant/70 tabular-nums">{r.totalVotes.toLocaleString()}</span>
                  {/* Trend icon */}
                  <span className={cn('w-12 text-right', getTrendColor(r.trend))}>
