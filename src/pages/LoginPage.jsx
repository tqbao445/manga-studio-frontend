...
// 5. ProtectedRoute trong App.tsx kiểm tra isAuthenticated → cho phép vào
//
    ...
    <div className="min-h-screen bg-[#fdf8f8] flex">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden items-center justify-center bg-primary">
        <div className="relative z-10 text-center px-12">
          ...
          <h1 className="font-display text-display-lg text-white mb-3">{APP_NAME}</h1>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <div className="lg:hidden flex items-center gap-3 mb-8 justify-center">
            ...
            <span className="font-display text-xl text-on-surface">{APP_NAME}</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
              <p className="text-center text-xs text-on-surface-variant mb-2">Quick login as:</p>
              <div className="flex flex-wrap gap-2 justify-center">
                <button type="button" onClick={() => quickLogin('ichikawa@manga.com')} className="text-xs px-2 py-1 border border-primary text-on-surface-variant hover:text-on-surface hover:bg-black/[0.02]">
                  Ichikawa (Mangaka)
                </button>
                <button type="button" onClick={() => quickLogin('sato@editor.com')} className="text-xs px-2 py-1 border border-primary text-on-surface-variant hover:text-on-surface hover:bg-black/[0.02]">
                  Sato (Editor)
                </button>
                <button type="button" onClick={() => quickLogin('tanaka@manga.com')} className="text-xs px-2 py-1 border border-primary text-on-surface-variant hover:text-on-surface hover:bg-black/[0.02]">
                  Tanaka (Assistant)
                </button>
                <button type="button" onClick={() => quickLogin('kimura@board.com')} className="text-xs px-2 py-1 border border-primary text-on-surface-variant hover:text-on-surface hover:bg-black/[0.02]">
                  Kimura (Board)
                </button>
              </div>
            </div>
