...
import { useSeriesList } from '../hooks/useMockData'
...
export function VoteEntryPage() {
  ...
  const series = seriesData?.content?.filter((s) => s.status === 'ONGOING' || s.status === 'COMPLETED') || []
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-on-surface-variant/70 mt-1">Enter reader vote data for the current period</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              />
              {periodLabel && (
                <p className="text-xs text-on-surface-variant mt-1">Period: {periodLabel}</p>
              )}
            </div>
              ...
              {series.map((s) => (
                <div key={s.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-on-surface-variant">{s.mangaka.displayName}</p>
                  </div>
                  <Input
                    type="number"
                    min="0"
          ...
          <CardContent>
            {anyVotes && periodLabel ? (
              <div className="space-y-2">
                <p className="text-xs text-on-surface-variant mb-3">Rankings will be calculated for {periodLabel}</p>
                {Object.entries(votes)
                  ...
                  .map(([seriesId, v], i) => {
                    ...
                    if (!s) return null
                    return (
                      <div key={seriesId} className="flex items-center gap-2 text-sm">
                        <span className="w-6 text-xs font-bold text-on-surface-variant tabular-nums">#{i + 1}</span>
                        <span className="flex-1 text-on-surface truncate">{s.title}</span>
                        <span className="text-on-surface-variant tabular-nums">{Number(v).toLocaleString()} votes</span>
                      </div>
                    )
                  })}
              </div>
