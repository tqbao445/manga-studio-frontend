...
import { useSeriesList, useChaptersBySeries } from '../hooks/useMockData'
...
function NewScheduleForm({ onClose }) {
  ...
  const chapters = (chaptersData?.content || []).filter((c) => c.status !== 'PUBLISHED')
  ...
  const handleSubmit = (e) => {
    ...
    const chapter = chapters.find((c) => c.chapterNumber === Number(chapterNumber))
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        {selectedSeries && (
          <p className="text-xs text-on-surface-variant mt-1">
            {selectedSeries.mangaka.displayName} · {selectedSeries.chapterCount} chapters · {selectedSeries.status}
          </p>
        )}
      </div>
      ...
      {selectedSeries && (
        <div className="space-y-1.5">
          ...
          <Input
            type="number"
            placeholder={chapters.length > 0 ? 'Or enter chapter number manually' : 'e.g. 19'}
            value={chapterNumber}
            onChange={(e) => setChapterNumber(e.target.value)}
            required
            min="1"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-on-surface-variant/70 mt-1">Manage publication schedules and releases</p>
        </div>
